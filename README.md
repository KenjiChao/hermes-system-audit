...
Current follow-up state: **audit complete; remediation not deployed; compatibility/remediation tests pending; browser cleanup not performed**. No dependency upgrades or browser shutdowns are represented as completed. Update the subpage's `state` data together with the evidence and copy when work advances; state labels in the hero, next-steps section and footer derive from that data.
